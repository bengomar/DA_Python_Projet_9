...
def home(request):
    ...
    user_following = UserFollow.objects.filter(user=request.user).values_list(
        "followed_user", flat=True
    )

    # Récupération des tickets liés aux utilisateurs suivis ou à l'utilisateur connecté
    tickets = Ticket.objects.filter(Q(user__in=user_following) | Q(user=request.user))
    ...
    reviews = Review.objects.filter(Q(user__in=user_following) | Q(user=request.user))
...
def delete_follow(request, user_id):
    ...
    follow = UserFollow.objects.get(user__exact=request.user, followed_user__exact=user)
    if follow:
        follow.delete()
    return redirect("follow_users")
...
def edit_ticket(request, ticket_id):
    ...
    return render(request, "core/edit_ticket.html", context=context)
